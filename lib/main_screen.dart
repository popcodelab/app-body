...
class _MainScreenState extends State<MainScreen> {
  ...
  void initState() {
    ...
    _screens = [
      const HomeScreen(), // Home screen with VehicleService and LocationService dependency
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: IndexedStack(
          ...
          children:
              _screens, // Use IndexedStack to maintain state of all screens
        ),
      ),
  ...
  BottomNavigationBar _buildBottomNavigationBar(BuildContext context) {
    return BottomNavigationBar(
      ...
      type: BottomNavigationBarType
          .fixed, // Ensure the bar items are always visible
      items: [
        _buildBottomNavigationBarItem(
          icon: Icons.home,
          label: AppLocalizations.of(context)?.nav_home ??
              'Home', // Fallback to 'Home' if localization fails
        ),
        _buildBottomNavigationBarItem(
          icon: Icons.search,
          label: AppLocalizations.of(context)?.nav_search ??
              'Search', // Fallback to 'Search' if localization fails
        ),
        _buildBottomNavigationBarItem(
          icon: Icons.settings,
          label: AppLocalizations.of(context)?.nav_settings ??
              'Settings', // Fallback to 'Settings' if localization fails
        ),
        _buildBottomNavigationBarItem(
          icon: Icons.person,
          label: AppLocalizations.of(context)?.nav_profile ??
              'Profile', // Fallback to 'Profile' if localization fails
        ),
      ],
    );
  }
